...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.pac.org -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_PAC-3.0.6.1-release-unsigned.apk \
        Pac-Electrum-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Pac-Electrum-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Pac-Electrum-{version}'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_PAC-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Pac-Electrum-{version}-release.apk'
