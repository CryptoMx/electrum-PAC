...
;Variables
  ...
  !define PRODUCT_NAME "Pac-Electrum"
...
Section "Pac-Electrum" SectionDE
  SetOutPath $INSTDIR
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\pac"
  ...
  ;Links pac: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\pac" "" "URL:pac Protocol"
  WriteRegStr HKCU "Software\Classes\pac" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\pac" "DefaultIcon" "$\"$INSTDIR\electrum-pac.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\pac\shell\open\command" "" "$\"$INSTDIR\electrum-pac-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Pac-Electrum Wallet"
