...
try:
    from electrum_pac.crypto import Hash, EncodeAES, DecodeAES
    from electrum_pac.bitcoin import (TYPE_ADDRESS, push_script, var_int, public_key_to_p2pkh, is_address,
                                      serialize_xpub, deserialize_xpub)
    from electrum_pac import ecc
    from electrum_pac.ecc import msg_magic
    ...
    from electrum_pac.keystore import Hardware_KeyStore
    from ..hw_wallet import HW_PluginBase
    from electrum_pac.util import print_error, to_string, UserCancelled
    from electrum_pac.base_wizard import ScriptTypeNotSupported, HWD_SETUP_NEW_WALLET
...
class DigitalBitbox_Client():
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Pac-Electrum-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf")
        msg = ('{"seed":{"source": "create", "key": "%s", "filename": "%s", "entropy": "%s"}}' % (key, filename, 'Digital Bitbox Pac-Electrum Plugin')).encode('utf8')
