...
class ExchangeBase(PrintError):
    ...
    def get_json(self, site, get_string):
        ...
        response = requests.request('GET', url, headers={'User-Agent' : 'Pac-Electrum'}, timeout=10)
        return response.json()

    def get_csv(self, site, get_string):
        url = ''.join(['https://', site, get_string])
        response = requests.request('GET', url, headers={
            'User-Agent': 'Pac-Electrum'
        })
...
class Bittrex(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('bittrex.com',
                             '/api/v1.1/public/getticker?market=%s-PAC' % ccy)
        quote_currencies = {}
...
class FxThread(ThreadJob):
    ...
    def exchange_rate(self, ccy=None):
    ...
    def format_amount_and_units(self, btc_balance, ccy=None):
        rate = self.exchange_rate(ccy)
        if ccy is None:
            ccy = self.ccy
        return '' if rate.is_nan() else "%s %s" % (self.value_str(btc_balance, rate, ccy), ccy)
    ...
    def value_str(self, satoshis, rate, ccy=None):
        if ccy is None:
            ccy = self.ccy
        return self.format_fiat(self.fiat_value(satoshis, rate), ccy)

    def format_fiat(self, value, ccy=None):
        if value.is_nan():
            return _("No data")
        if ccy is None:
            ccy = self.ccy
