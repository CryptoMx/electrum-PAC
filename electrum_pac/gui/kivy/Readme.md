...
The Kivy GUI is used with Pac-Electrum on Android devices. To generate an APK file, follow these instructions.

## 1. Preliminaries

Make sure the current user can write `/opt` (e.g. `sudo chown username: /opt`).

We assume that you already got Pac-Electrum to run from source on this machine,
...
p4a is used to package Pac-Electrum, Python, SDL and a bootstrap Java app into an APK file. 
We patched p4a to add some functionality we need for Pac-Electrum. Until those changes are
...
In the `gui/kivy` directory of Pac-Electrum, run `make theming`.

## 6. Download Pac-Electrum dependencies
```sh
