...
SYSTEM_CONFIG_PATH = "/etc/electrum-PAC.conf"
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-PAC.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-PAC.conf. please install ConfigParser"
            return
